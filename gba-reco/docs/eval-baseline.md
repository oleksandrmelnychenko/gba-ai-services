...
byRegion)`). It is now forwarded to gba-reco as opt-in `region_scope` (commit in
`gba-server`: `af31ff2`), while the default remains off.
...
Recommendation: HOLD. Do not adopt region scoping as a default. The .NET DTO/controller path is
wired so the existing toggle is no longer dead, but it must remain opt-in and gated behind product
