...
def backtest_producer(producer_id: int, as_of: str, eval_window_days: int | None = None,
    ...
    for it in plan.items:
        if it.urgency == Urgency.CRITICAL:
            critical_items += 1
    ...
    return BacktestResult(
        ...
        critical_share=round(critical_items / total_items, 4) if plan.items else 0.0,
...
def main() -> None:
    ...
    print(f"  critical_share       : {r.critical_share:.3f}  "
          f"({r.critical_items}/{r.total_items} planned items)")
